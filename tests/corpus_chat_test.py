...
class TestRegulationChat:
    ...
    def test__add_rag_data_to_question(self):
        dfns = []
        dfns.append(["GDPR", "My definition from GDPR"])
        dfns.append(["Article_30_5", "My definition from article_30_5"])
        df_definitions = pd.DataFrame(dfns, columns = ["document", "definition"])
        sections = []
        ...
        expected_text = f'Question: user asks question\n\nExtract 1:\nMy definition from GDPR\nExtract 2:\nMy definition from article_30_5\nExtract 3:\nMy Section 1 from GDPR\nExtract 4:\nMy Section 2 from GDPR\nExtract 5:\nFake section from Article_30_5\n'
    ...
    def test__check_response(self):
        ...
        df_definitions = pd.DataFrame(dfns, columns = ["document", "definition"])
        sections = []
        sections.append(["GDPR", "1", "My Section 1 from GDPR that refers to Article 16(1)(b)"])
        sections.append(["GDPR", "2", "My Section 2 from GDPR"])
        sections.append(["Article_30_5", "A.4(i)", "Fake section from Article_30_5 that makes a reference to Article 30"])
        df_search_sections = pd.DataFrame(sections, columns = ["document", "section_reference", "regulation_text"])
        ...
        assert check_result["llm_followup_instruction"] == "Your response, did not begin with one of the keywords, 'ANSWER:', 'SECTION:' or 'NONE:'. Please re-write your answer in the required format."
        ...
        # # Check SECTION: 
        path = "SECTION:"
        references = "Extract 1, Reference Article 16(1)(b)"
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert check_result["success"]
        assert check_result["path"] == path
        assert check_result["document"] == 'GDPR'
        assert check_result["section"] == '16(1)(b)'

        path = "SECTION:"
        references = "Extract (3), Reference Article 16(1)(b)"
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert not check_result["success"]
        assert check_result["path"] == path
        ...
        path = "SECTION:"
        references = "Extract 6, Reference Article 16(1)(b)"
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert not check_result["success"]
        assert check_result["path"] == path
        ...
        path = "SECTION:"
        references = "Extract 3, Reference 16(1)(b)"
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert check_result["success"]
        assert check_result["path"] == path
        ...
        path = "SECTION:"
        references = "Extract 5, Reference Article 16(1)(b)" # doc 5 is article_30_5 but it may have a GDPR reference
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert check_result["success"]
        assert check_result["path"] == path
        ...
        path = "SECTION:"
        references = "Extract 5, Reference 16(1)(b)" # doc 5 is article_30_5 but it may have a GDPR reference
        response = f"{path} {references}"
        check_result =    self.chat._check_response(response, df_definitions=df_definitions, df_sections=df_search_sections)
        assert check_result["success"]
        assert check_result["path"] == path
        ...
        assert not check_result["success"]
        assert check_result["path"] == path
        assert check_result["llm_followup_instruction"] == "The reference XXX does not appear to be a valid reference for the document. Try using the format (\d{1,2})(?:\((\d{1,2})\))?(?:\(([a-z])\))?"
    ...
    def test_resource_augmented_query(self):
        ...
        question = "Are there exemptions from GDPR for small companies?"
        testing = True
        manual_responses_for_testing = []
        manual_responses_for_testing.append("SECTION: Extract 1, Reference 30(5)")

        result = self.chat.resource_augmented_query(user_question = question,
                                                        df_definitions = relevant_definitions, 
                                                        df_search_sections = relevant_sections,
                                                        testing = testing,
                                                        manual_responses_for_testing = manual_responses_for_testing)
        ...
        assert result["document"] == 'GDPR'
        assert result["section"] == '30(5)'
        ...
        manual_responses_for_testing.append("SECTION: Extract 1, Reference 30(5)") # but after marking its own homework it behaves
    ...
    def test_reformat_assistant_answer(self):
        ...
        raw_response = 'Some random text here.\nReference:\nDefinition 4(2) from General Data Protection Regulation\nThe definitions in WORKING PARTY 29 POSITION PAPER on the derogations from the obligation to maintain records of processing activities pursuant to Article 30(5) GDPR\nSection 1 from General Data Protection Regulation\nSection A.4(i) from WORKING PARTY 29 POSITION PAPER on the derogations from the obligation to maintain records of processing activities pursuant to Article 30(5) GDPR\n'
    ...
    def test_user_provides_input(self):
        ...
        output_response = 'Yes, there is a derogation for organizations with fewer than 250 employees with regard to record-keeping under Article 30 of the GDPR. However, this derogation is not absolute and does not apply to processing that is likely to result in a risk to the rights and freedoms of data subjects, processing that is not occasional, or processing that includes special categories of data or personal data relating to criminal convictions and offences. Therefore, small companies may be exempt from the record-keeping obligation if they meet the criteria specified in Article 30(5).\nReference:\nSection 30(5) from General Data Protection Regulation'
    ...
    def test_add_section_to_resource(self):
        ...
        assert df_updated.iloc[0]['section_reference'] == "A.4(i)"
        assert df_updated.iloc[1]['section_reference'] == '50'


        # check if the section string only comes from the definitions, then replace all the df_search_data with only he new entry
        result = {"success": True, "path": "SECTION:", "extract": 1, "document": 'GDPR', "section": "50"}
        df_updated = self.chat.add_section_to_resource(result, df_definitions, df_search_sections)
        assert len(df_updated) == 1
        assert df_updated.iloc[0]['section_reference'] == '50'
